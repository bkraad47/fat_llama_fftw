...
```json
{
  ...
  "quality": [
    { "file": "fat_llama_fftw/audio_fattener/feed.py", "line": 42, "rule": "E501", "description": "line too long (92 > 79 characters)" }
  ],
...
- Fallback (matches CI in `.github/workflows/tests.yml`): `python -m unittest discover -s fat_llama_fftw/tests`
...
- If a failure is environmental (e.g. `pyfftw` fails to build or import because the underlying FFTW system library isn't installed/discoverable), report it as a failure with that reason rather than trying to work around it.
...
- Test execution is scoped to `fat_llama_fftw/tests/**` — never run or introduce tests/scripts outside that directory.
