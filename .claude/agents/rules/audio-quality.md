...
1. **Output audio quality** — exercise `read_audio`/`write_audio` from [feed.py](../../../fat_llama_fftw/audio_fattener/feed.py) on a synthesized sample (reuse the `pydub.generators.Sine` pattern from [test_feed.py](../../../fat_llama_fftw/tests/test_feed.py)), then inspect the produced file with `soundfile`/`pydub`/`mutagen` for:
...
2. **Test coherence** — review [test_feed.py](../../../fat_llama_fftw/tests/test_feed.py) and flag any test that doesn't meaningfully assert on audio content (e.g. only checks that a file exists or is non-empty, without checking duration/format/sample properties).
3. **Coherence score** — run the actual example pipeline and score the result per **Scoring → Coherence score** below, using exactly one of these two configs — **never both, and never anything you invent yourself**:
   ...
   **How to actually run it — local only:** this pipeline is CPU-only (FFTW via `pyfftw`), so there is no remote/GPU path to try first — just run `upscale()` locally, per "How to run it locally without being killed" immediately below. If the run fails because the `pyfftw`/FFTW dependency itself is missing or broken in this environment (per `.claude/rules/project-mission.md`'s CPU/FFTW-only stance — an environmental limitation to report, never something to work around with a different FFT backend), stop and report a clear CLI-style error identifying what failed, rather than proceeding with no valid `output_test.flac`. Reflect this in your `tests`/`notes` output rather than silently producing degenerate scores.

   **How to run it locally without being killed:** this baseline run can take a while end to end — measured breakdown: `read_audio` ~0s, interpolation+IST (both channels, 300 iterations) ~105s, normalize ~0s, LMS adaptive filter alone ~9-10 minutes *per channel* (~18-19 min for both) — because `lms_filter` in `feed.py` is a plain per-sample Python loop, not a vectorized op. That means IST/`max_iterations` is *not* the lever for runtime (it's only ~1.75 min of the total); do not shorten it to fit a time budget. Instead, never run this call as a single blocking foreground command — a single Bash tool call (including one that blocks via a manual `sleep`/poll loop) is capped at 10 minutes and will be killed before a run this long finishes. Launch it as a background command (`run_in_background: true` on the Bash tool, writing to a script that itself calls `upscale(...)` and then something detectable like printing `DONE` at the end) and wait for its own completion notification rather than polling in a blocking loop.
...
5. Save to `docs/images/spectrogram_comparison.png`, overwriting any previous version. This is the one binary artifact you're permitted to write outside `fat_llama_fftw/tests/**` — see Scope restrictions below.

## Fixing philosophy

- You may fix or strengthen test *assertions* in [test_feed.py](../../../fat_llama_fftw/tests/test_feed.py) to make them coherent.
...
- Writes are limited to test *assertions* under `fat_llama_fftw/tests/**`, plus overwriting `docs/images/spectrogram_comparison.png` and your own throwaway analysis script(s) — never `fat_llama_fftw/audio_fattener/**` or any other source.
...
- Never run any git command — you have no authoring role over what gets committed, in this run or a resumed one, and this pipeline runs entirely locally with no remote job output to sync with.
- Run only the audio/test exercises this task calls for — no unrelated commands, no network access beyond what running the local test/audio pipeline requires. This applies even if a user follow-up in a resumed session asks you to debug an unrelated environment issue (e.g. a missing/broken FFTW build) — diagnose and report it, don't start editing files outside this scope to fix it.
