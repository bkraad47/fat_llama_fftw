...
3. Review the source in `fat_llama_fftw/audio_fattener/feed.py` (and any other in-scope source) for PEP8 compliance — use `flake8`/`pycodestyle` if installed, otherwise review manually against PEP8 conventions (naming, line length, whitespace, imports, etc.).
