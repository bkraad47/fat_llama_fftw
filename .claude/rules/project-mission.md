...
This `fat_llama_fftw` package targets **CPU-accelerated processing via FFTW (through `pyfftw`)** — that's the primary and only path this project's skills/agents build and test against, not a fallback. A CUDA-accelerated variant exists as a separate package (`fat-llama`, linked from `README.md`'s Installation section) but is out of scope here: don't add CUDA/CuPy code paths, GPU-only branches, or make FFTW/`pyfftw` optional in `fat_llama_fftw` itself unless the user explicitly asks for that as a distinct task. If a target environment is missing the `pyfftw`/FFTW dependency (e.g. it fails to build or import), treat that as an environmental limitation to report (per `code-quality.md`/`scientific-coding.md`), not something to work around by reaching for a different FFT backend.
