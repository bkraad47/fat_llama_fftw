...
- Beyond those two universal exclusions, each skill/agent's own rules file states its write scope. As of this writing:
  - `code-tester-reviewer` (agent) — read-only; runs tests scoped to `fat_llama_fftw/tests/**`; never writes to source or tests. See `.claude/agents/rules/code-quality.md`.
  - `audio-quality-checker` (agent) — may edit test *assertions* under `fat_llama_fftw/tests/**`; may also write/overwrite the spectrogram comparison image at `docs/images/spectrogram_comparison.png` and any throwaway analysis script it creates to produce its scores; never touches source under `fat_llama_fftw/audio_fattener/**`. See `.claude/agents/rules/audio-quality.md`.
  - `generate-code` (agent, and the skill that dispatches it) — the only role permitted to change production source; may edit source and tests under `fat_llama_fftw/**`. See `.claude/agents/rules/scientific-coding.md`.
