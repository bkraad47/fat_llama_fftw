...
**File:** fat_llama_fftw/audio_fattener/feed.py:143
**Kind:** function
**Description:** New in cycle 1 — zeroes every FFT bin above `original_nyquist` (via `pyfftw` `rfft`/`irfft`) and returns the filtered signal. Wired into `upscale()` as the final processing stage, enforcing `.claude/rules/project-mission.md`'s hard constraint that no audible content above the original source's Nyquist frequency may survive in the output — the zero-order-hold interpolation in `new_interpolation_algorithm` otherwise images the spectrum above that point. Measured: peak above-Nyquist content on `input_test.mp3`'s output dropped from -52.1 dB to -145.7 dB relative to the output's own peak.
**Parameters:**
...
**File:** fat_llama_fftw/audio_fattener/feed.py:161
**Kind:** function
**Description:** The package's public entry point (README's documented API). Reads the source file, computes an upscale factor from the target vs. original bitrate, runs interpolation+IST per channel via `upscale_channels`, auto-scales each channel's amplitude back to the original's peak level, normalizes, then (cycle-1 addition) runs `apply_nyquist_cutoff` per channel as the final stage before writing. Note: this signature still has no `toggle_*` flags (normalize/autoscale/adaptive-filter) and no LMS adaptive filter step — auto-scaling, normalization, and the Nyquist cutoff always run unconditionally as the last stages.
**Parameters:**
