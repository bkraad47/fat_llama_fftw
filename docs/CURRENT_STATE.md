...
Snapshot taken against candidate `v-2.0.0/iter-1` (commit `7230447`) — after `iterate-fat-llama`'s cycle 1 fix, which ported the CUDA sibling repo's (`bkraad47/fat_llama` `v-2.0.0` branch) frequency-selective + envelope-gated IST peak-capping refinements into `_cap_ist_changes_to_baseline_peak`.
...
### `_cap_ist_changes_to_baseline_peak(expanded_channel, ist_changes, max_rounds=20, dominant_band_ratio=0.002, safety_margin=1.2) -> np.ndarray`
**File:** fat_llama_fftw/audio_fattener/feed.py:453
**Kind:** function (private helper)
**Description:** Bounds how much `perform_ist_iteration`'s peak-relative boost of a channel's dominant (usually low-frequency) content is allowed to inflate the combined (interpolation + IST) signal's peak above the pre-IST baseline's own peak — an inflated peak here is not "corrected" by `upscale()`'s later autoscale/normalize stages (each a per-channel scalar, together mathematically inert on the final output), it determines how hard *every* frequency, not just what IST touched, gets divided down at final normalization. **Rewritten this cycle** (ported from the CUDA sibling repo `bkraad47/fat_llama`'s `v-2.0.0` branch — pure numpy/FFT logic, no CUDA-specific mechanism) with two refinements on top of the original whole-signal uniform-scalar shrink (still present as the fallback path): (1) **frequency-selective dominant/residual split** — `ist_changes`'s own `rfft` spectrum is split into a "dominant" component (bins `>= dominant_band_ratio` of the spectrum's own peak magnitude) and a "residual" component; only the dominant component is iteratively shrunk (the original bounded multiplicative-shrink loop, reused, scoped to this component) toward bounding the combined peak, leaving residual (e.g. genuinely-added quiet/high-frequency detail) untouched — measured to fix the old uniform shrink's suppression of a quiet band down to ~4% of its own magnitude; (2) **envelope-gated correction** — the dominant-band correction (`(1 - dominant_scale) * dominant_component`) is tapered by `gate = clip(_local_peak_envelope(expanded_channel) / baseline_peak, 0, 1)`, so a genuinely non-stationary channel's quiet onset/fade-in (which a single whole-buffer scalar correction would otherwise "unmask" — measured up to +18.5dB frame-RMS elevation on the CUDA sibling — because the dominant/residual split's disjoint frequency components reconstruct the true quiet shape only via a near-exact time-domain cancellation between them) receives little to none of the correction. Falls back to one additional whole-signal uniform-shrink pass (the original method, gated the same way) on top of the frequency-selective candidate only if that candidate's own combined peak still exceeds `baseline_peak * safety_margin` (handles a pathological `dominant_band_ratio` that classifies nothing as dominant — degenerates exactly to the pre-existing uniform-shrink behavior in that case). `max_rounds=20` (unchanged), `dominant_band_ratio=0.002` and `safety_margin=1.2` (new) were empirically tuned on adversarial synthetic fixtures.
**Parameters:**
...
**Kind:** class
**Description:** `unittest.TestCase` covering every function in `feed.py`, 47 test methods as of this cycle. Notable groups: I/O (`read_audio`/`write_audio`, including a real on-disk roundtrip test), interpolation (bandlimited-ness, identity/passthrough edge cases, no-imaging-above-Nyquist), IST core (thresholding, chaining, convergence early-exit, DC-bin exclusion, WOLA block/no-discontinuity behavior, scale-invariance), the peak cap (`_cap_ist_changes_to_baseline_peak` — 6 tests as of this cycle, see below), the Nyquist cutoff (removal + multi-threading), and full `upscale()` wiring/edge cases (bitrate clamping, never-exceeds-full-scale, a real end-to-end on-disk FLAC write).
**Usage:**
```python
...
**Kind:** method
**Description:** **New this cycle.** Broadband, raised-cosine fade-in synthetic channel run through the real `new_interpolation_algorithm` → `iterative_soft_thresholding` → `_cap_ist_changes_to_baseline_peak` pipeline; asserts the onset window's post-cap RMS elevation vs. the pre-IST interpolation baseline stays under a small bound — the direct regression test for `_local_peak_envelope`'s gating (an ungated frequency-selective correction was measured to "unmask" a much larger elevation here).
**Returns:** `None` (assertion-based).
