...
| Metric | Score | Notes |
|---|---|---|
| Coherence (upscale quality, 0-10) | 5 | clean on clipping (0.0229% >0.999), dropouts (0 unmatched runs), invalid samples (0 NaN/Inf) and discontinuities (99.9th-pct \|diff\| 0.0581818 vs reference 0.0581818, ratio 1.000), but capped at 5 by the above-Nyquist precondition: -52.1 dB peak at 37231 Hz and -57.1 dB mean in 30-50 kHz against a -112.3 dB in-band noise floor, with no measurable added detail below 22050 Hz (+8.7e-8 dB in the quiet 14-22 kHz band) |
| Spectral deviation vs. reference FLAC (0-10) | 10.0 | convergence=1.0000, correlation=1.0000 against input_test.flac - degenerate: output_test.flac matches input_test.flac to within 2.38e-7 (1 LSB of 24-bit) because that repo reference is itself a prior output of this same pipeline, so this measures reproducibility, not fidelity to a lossless master |
