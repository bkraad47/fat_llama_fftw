...
| Metric | Score | Notes |
|---|---|---|
| Coherence (upscale quality, 0-10) | 8 | clean on every check - 0 NaN/Inf, 0 unmatched dropouts, 99.9th-pct \|diff\| 0.0152 vs reference 0.0582 (smoother, not clickier), clipping fraction 0.0169% identical to the reference's own, and above-Nyquist content now peaks at -147.9 dB against a -137.2 dB in-band noise floor (down from -57.2 dB pre-fix) - but no measurable added detail below 22050 Hz yet (all band deltas +0.000 dB), so this is a safe, transparent upscale rather than a 10 |
| Spectral deviation vs. reference FLAC (0-10) | 9.9 | convergence=0.9842, correlation=0.9999 against input_test.flac; the 1.6% convergence shortfall is entirely the above-Nyquist imaging the reference itself still carries (it's a pre-Nyquist-fix prior output of this same pipeline) being correctly removed from the new output - a desired deviation, not degradation |
