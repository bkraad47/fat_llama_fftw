...
| Metric | Score | Notes |
|---|---|---|
| Coherence (upscale quality, 0-10) | 9.0 | every check clean (peak exactly 1.0, 0 dropouts/NaN/Inf, discontinuities 12.3dB below the reference's own, above-Nyquist energy -134.5/-134.8dB); held under 10 by two open findings: no IST-attributable added detail below the original Nyquist (a long-standing gap, not new), and a ~3.6dB short-time dynamic-range compression from the envelope gate's quiet/loud asymmetry (a new finding, flagged as a candidate for a future improvement cycle) |
| Spectral deviation vs. reference FLAC (0-10) | 9.8 | convergence=0.9575, correlation=0.9992 against input_test.flac |
