...
| Metric | Score | Notes |
|---|---|---|
| Coherence (upscale quality, 0-10) | 6 | clipping, dropouts, invalid samples, discontinuities, above-Nyquist content, and DC offset are all clean and mostly beat the reference (DC now -100.6 dBFS vs. the reference's -95.7 dBFS, fully fixing the prior cycle's regression) - but the new block/windowed IST's overlap-add reconstruction injects a periodic broadband artifact at its own ~75 Hz block rate instead of restored detail (crest +17 dB over quiet passages, envelope-programme correlation only 0.121), and spectral convergence dipped 0.9058 -> 0.8087 |
| Spectral deviation vs. reference FLAC (0-10) | 9.0 | convergence=0.8087, correlation=0.9862 against input_test.flac; the convergence drop comes from a ~4 dB midband tilt plus new 19-22.05 kHz block-rate splatter raising the output's own peak-normalisation reference |
