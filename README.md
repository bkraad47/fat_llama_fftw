...
| Metric | Score | Notes |
|---|---|---|
| Coherence (upscale quality, 0-10) | 8.5 | every hard check clean and mostly beats the reference (peak exactly 1.0, DC/above-Nyquist/discontinuities/hop-boundary all clean); the high-shelf attenuation vs. a no-IST control is down to a sub-audible -0.4dB (from ~1.3dB) - but IST's added detail is now confined to bands that already dominate the signal (20-2000Hz, +0.02 to +0.28dB) with exactly 0.000dB contribution above 2kHz, so this remains a safe, transparent upscale rather than one that restores missing/congested high-frequency detail |
| Spectral deviation vs. reference FLAC (0-10) | 9.9 | convergence=0.9723, correlation=0.9998 against input_test.flac; note input_test.flac is itself effectively a no-IST/plain-upsample control, so this should not be read as a quality ceiling |
