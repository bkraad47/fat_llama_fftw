...
| Metric | Score | Notes |
|---|---|---|
| Coherence (upscale quality, 0-10) | 7 | the cycle-4 block-hop-rate artifact is confirmed gone (quiet-frame energy now matches loud frames) and all integrity checks are clean, but the upscale is not yet transparent: minor write-stage clipping (19 samples, since fixed in cycle 5), DC offset rising to -56.1 dBFS from the source's -95.7 dBFS (since fixed in cycle 5), a ~6 dB LF-to-HF tonal tilt, and no measurable added detail below the original Nyquist |
| Spectral deviation vs. reference FLAC (0-10) | 8.9 | convergence=0.7954, correlation=0.9888 against input_test.flac; the high correlation with lower convergence reflects the tonal tilt above rather than a structural difference |
