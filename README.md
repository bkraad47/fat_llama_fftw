...
| Metric | Score | Notes |
|---|---|---|
| Coherence (upscale quality, 0-10) | 7 | clipping, dropouts, invalid samples, discontinuities, and above-Nyquist content all clean (peak -152.9 dB, 36-49 dB below the in-band noise floor) - but there is still no added detail below 22050 Hz, and the cycle-2 IST fix now injects a DC offset (+41.7 dB vs reference) plus a ~49 Hz drone across the whole file, dipping spectral correlation from 0.9999 to 0.9957 |
| Spectral deviation vs. reference FLAC (0-10) | 9.5 | convergence=0.9058, correlation=0.9957 against input_test.flac; both down from the prior cycle because the injected DC/49 Hz component raises the output's own peak STFT magnitude, scaling all other content down under per-max normalisation |
