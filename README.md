...
| Metric | Score | Notes |
|---|---|---|
| Coherence (upscale quality, 0-10) | 9.9 | every check clean (clipping ~72x cleaner than the reference, 0 dropouts/NaN/Inf, discontinuities -12.3dB below the reference's own, above-Nyquist energy -137.5dB) plus measurable added detail concentrated in the bands the MP3 source is most starved in (+0.4 to +2.0dB at 8-20kHz); held under 10 only by a residual -0.16 to -0.23dB mid-band (300Hz-4kHz) dip and a -5.8dB shortfall at 20-22.05kHz vs. the reference (a band the source has essentially nothing in) |
| Spectral deviation vs. reference FLAC (0-10) | 9.8 | convergence=0.9685, correlation=0.9998 against input_test.flac |
