...
| Metric | Score | Notes |
|---|---|---|
| Coherence (upscale quality, 0-10) | 8.5 | every hard check passes decisively and beats the reference (clipping, dropouts, DC offset, above-Nyquist energy all cleaner than input_test.flac itself; quiet-frame levels 3-4 dB below the reference, so no noise-floor rise) - held back from higher only by the newly-measurable 19-22.05kHz energy being flat/noise-shaped splatter rather than reconstructed detail, and a residual +5.6dB block-hop-boundary curvature bump |
| Spectral deviation vs. reference FLAC (0-10) | 9.0 | convergence=0.8038, correlation=0.9885 against input_test.flac; driven by a ~2.5dB high-shelf attenuation IST introduces above ~1kHz relative to a no-IST control - note input_test.flac is itself effectively a no-IST/plain-upsample control, so this score should not be read as a quality ceiling |
