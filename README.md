...
| Metric | Score | Notes |
|---|---|---|
| Coherence (upscale quality, 0-10) | 8 | every hard check clean (peak exactly 1.0, DC/above-Nyquist/discontinuities all on par with or better than input_test.flac); the block-hop-boundary artifact is now confirmed fully gone at the output level, and the 19-22.05kHz band is faithful MP3-source content, not pipeline splatter - but the ~1.3dB high-shelf attenuation vs. a no-IST control persists unchanged, and no measurable added spectral detail exists anywhere in the output yet |
| Spectral deviation vs. reference FLAC (0-10) | 9.6 | convergence=0.9281, correlation=0.9991 against input_test.flac; note input_test.flac is itself effectively a no-IST/plain-upsample control, so this should not be read as a quality ceiling |
